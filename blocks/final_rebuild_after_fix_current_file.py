...
if res.returncode == 0:
    ...
    if os.path.exists(exe):
        ...
        print(f"\n🔧 修复内容：")
        print(f"  1. ✅ 将错误的 `self.current_file` 改为 `self.current_book_path`")
        print(f"  2. ✅ 修复了所有缩进问题和重复定义问题")
        print(f"  3. ✅ 语法检查通过")
        print(f"\n💡 错误原因：")
        print(f"  代码中错误使用了不存在的属性 `current_file`，应该是 `current_book_path`")
else:
    print(f"\n❌ 打包失败：\n{res.stderr}")